<template>
  <div>
    ...
    <div class="flex items-center justify-between mb-6">
      ...
      <button @click="openBuildingModal('add')" class="bg-primary-600 hover:bg-primary-700 text-white px-4 py-2 rounded-lg font-medium shadow-sm transition-colors flex items-center space-x-2">
        <Plus class="w-4 h-4" />
    ...
    <div class="bg-white p-4 rounded-xl shadow-sm border border-slate-200 mb-8 flex flex-wrap items-center justify-between gap-4">
      <div v-if="buildings.length > 0" class="flex items-center space-x-4">
        <Filter class="w-5 h-5 text-slate-500" />
        <span class="font-semibold text-slate-700">เลือกอาคาร:</span>
        <select v-model="selectedBuildingId" class="border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 font-medium text-slate-800 outline-none w-48 bg-slate-50">
      ...
      <div v-else class="text-slate-500 font-medium italic">
    ...
    <div v-if="currentBuilding" class="space-y-8">
      ...
      <div v-for="floor in sortedFloors" :key="floor.id" class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 relative overflow-hidden">
        ...
        <!-- Room Cards Grid -->
        <div class="grid gap-6 grid-cols-[repeat(auto-fill,minmax(340px,1fr))]">
          <div v-for="room in floor.rooms" :key="room.number" 
               ...
               :class="{
                 'bg-primary-50/40 border-primary-200': room.status === 'Occupied',
            ...
            <div class="flex justify-between items-start mb-4">
              <h4 class="text-2xl font-bold tracking-tight" :class="{
                'text-primary-700': room.status === 'Occupied',
              ...
              <span class="px-2.5 py-1 text-xs font-semibold rounded-full shadow-sm whitespace-nowrap"
                    :class="{
                      'bg-primary-100 text-primary-700': room.status === 'Occupied',
                      'bg-slate-100 text-slate-600': room.status === 'Vacant',
                      'bg-amber-100 text-amber-700': room.status === 'Maintenance'
                    }">
                {{ room.status === 'Occupied' ? 'มีผู้เช่า' : room.status === 'Maintenance' ? 'ไม่พร้อมเข้าอยู่' : 'ว่าง' }}
              </span>
            </div>
...
const sortedFloors = computed(() => {
  ...
  return [...currentBuilding.value.floors].sort((a, b) => 
